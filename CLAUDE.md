...
**Performance (mobile-first):** the **Inter** font is self-hosted via a single `@fontsource-variable/inter` import in `Layout.astro`'s frontmatter (the variable font covers the full 100–900 weight range in one file, so `font-medium`/`semibold`/`bold`/`extrabold` all resolve correctly). Its family name is `Inter Variable`, which leads the `sans` stack in `tailwind.config.mjs` (`['Inter Variable', 'Inter', …]`). There is **no** `fonts.googleapis.com` link (do not re-add one), and do not split it back into per-weight `@fontsource/inter` imports (that adds extra woff2 requests to the critical chain). `astro.config.mjs` sets `build: { inlineStylesheets: 'always' }`, so all bundled CSS (including the `@font-face` rule) ships inline in each page's `<head>` rather than as a render-blocking external stylesheet. These settings target the PageSpeed "render-blocking requests" / critical-request-chain audits — keep them when touching fonts/CSS.
